...
const emailService = {
	...
	async send(c, params, userId) {
		...
		// Validate attachment limits before calling an external provider.
		// Previously the email could be sent successfully and only then fail while
		// saving attachment metadata, which made retries capable of sending duplicates.
		...
		if (useCloudflareEmail && !allInternal && receiveEmail.length > 50) {
			throw new BizError(t('cfRecipientLimit'), 400);
		}
	...
	async sendByCloudflareEmail(c, params) {
		...
		try {
			...
			let httpStatus = 502;
			if (['E_RATE_LIMIT_EXCEEDED', 'E_DAILY_LIMIT_EXCEEDED'].includes(code)) httpStatus = 429;
			if ([
				...
				'E_HEADERS_TOO_MANY'
			].includes(code)) httpStatus = 400;
			throw new BizError(`Cloudflare Email Service [${code}]: ${error?.message || 'send failed'}`, httpStatus);
		}
