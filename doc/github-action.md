...
2. 创建新的 API 令牌
3. 选择“编辑 Cloudflare Workers”模板，并额外添加 **Account → D1 → Edit/Write** 权限（工作流会执行 `wrangler d1 list/create`；如你始终手工提供现成 D1 ID，至少仍建议保留 D1 Read）。再参照下表添加其他所需权限
...
**运行工作流**
1. 然后在Action页面手动运行工作流，后续同步上游后会自动部署到 Cloudflare Workers。数据库初始化优先使用 `POST https://你的项目域名/api/init`，并在请求头中设置 `X-Init-Secret: 你的jwt_secret`。旧的 `/api/init/你的jwt_secret` GET 路径仅为兼容保留，不建议继续使用，因为 URL 更容易出现在访问日志和历史记录中。
